...
class Agile:
    def __init__(self, betas, Nr=8):
        ...
        self.beta_matrix = np.exp(-2j * np.pi * self.d * np.arange(Nr) * np.sin(betas * np.pi / 180).reshape(-1, 1))
    ...
    def call(self, X):
        ...
        alpha_matrix = np.exp(-2j * np.pi * self.d * np.arange(self.Nr) * np.sin(alphas.reshape(-1, 1)))
