...
def prepare_data_for_cnn(file_path='Combined_Beam_IQ.csv', num_beams=4):
    ...
    X = np.zeros((len(valid_indices), 2, 512))  # 2 channels for real and imaginary parts
    y = labels
    ...
    for i in range(len(valid_indices)):
        ...
        for b_idx, beam in enumerate(selected_beams):
            ...
            X[i, 0, current_position:current_position+actual_samples] = np.real(complex_signal[:actual_samples])
            X[i, 1, current_position:current_position+actual_samples] = np.imag(complex_signal[:actual_samples])
...
if __name__ == "__main__":
    X, y = prepare_data_for_cnn(file_path='Combined_Beam_IQ.csv', num_beams=4)
    ...
    print("Using same data for training and testing")
    
    from agile_cnn import AgileCNN, train_agile_model, evaluate_and_plot
    
    model = AgileCNN(num_beams=4, num_classes=65).to(device)
    
    model = train_agile_model(
        model,
        data_loader,  # Same loader for training
        data_loader,  # Same loader for validation
        epochs=200,   # More epochs to see if it can memorize the data
        device=device,
        learning_rate=0.0005
    )
    
    accuracy, mae, _, _, _ = evaluate_and_plot(model, X, y, 4, device)
