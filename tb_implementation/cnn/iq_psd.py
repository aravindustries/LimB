...
def apply_rayleigh_fading_and_noise(data, snr_db=10):
    ...
    rayleigh_fading = (
        np.random.normal(0, 1, (num_angles, num_beams, num_values))
        + 1j * np.random.normal(0, 1, (num_angles, num_beams, num_values))
    ) / np.sqrt(2)
    snr = 10 ** ((snr_db) / 10)
    ...
    noise = np.sqrt(noise_power / 2) * (
        np.random.randn(num_angles, num_beams, num_values)
