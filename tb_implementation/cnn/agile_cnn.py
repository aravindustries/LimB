...
class AgileCNN(nn.Module):
    def __init__(self, num_beams, num_classes=91):
        super().__init__()
...
def apply_agile_beam_switching(X, y, num_beams, spread=32, Nr=4, d=0.55):
    ...
    beta_matrix = np.exp(
        -2j * np.pi * d * np.arange(Nr) * np.sin(betas * np.pi / 180).reshape(-1, 1)
    )

    k = 32
    alphas = np.array(range(-k, k + 1)) * np.pi / 180
    alpha_matrix = np.exp(
        -2j * np.pi * d * np.arange(Nr) * np.sin(alphas.reshape(-1, 1))
    )
    ...
    for i in range(num_samples):
        ...
        for b in range(num_beams):
            ...
            beamformed_segment = (
                beam_weights @ complex_signal[:, start_idx:end_idx] * beam_gain
            )
